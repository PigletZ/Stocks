...
注意：Tushare 部分科目存在新旧两套列名（如 c_rec_fr_invsts / c_disp_withdrwl_invest），
同一概念映射为同一中文名；不同时期报告可能分别使用不同列名填充。
"""
...
CASHFLOW_FIELDS = {
    ...
    "n_disp_subs_oth_biz": "处置子公司及其他营业单位收到的现金净额",
